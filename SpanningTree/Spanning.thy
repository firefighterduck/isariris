...
shows "(graph_ctxt \<kappa> g Mrk) \<^emph> (own_graphUR q fmempty) \<^emph> (cinv_own upd_cinv \<kappa> k) \<turnstile>
  ...
  apply (auto simp: try_mark_def graph_ctxt_def subst'_def 
    intro!: wp_pure[OF wp_inG pure_exec_beta] wp_let_bind'[OF wp_inG, where C=Fst])
  \<comment> \<open>Open the graph invariant for the atomic load instruction.\<close>
  apply (iMod rule: cinv_acc[OF cinvInG.inG_axioms invInG.inG_axioms subset_UNIV])
  ...
  apply (iApply rule: wp_load[OF wp_inG, where ?l = x, simplified])
  ...
  apply (iApply rule: wp_pure_let[OF wp_inG pure_exec_fst, simplified], simp add: subst'_def)
  ...
  subgoal
    ...
    apply (iApply rule: wp_cmpxchg_fail[OF wp_inG, simplified, where ?l = m'], auto simp: vals_compare_safe_def)
    ...
    apply (iWP rule: wp_pure[OF _ _ _ pure_exec_snd])
    by (entails_substR rule: wp_value[OF wp_inG], iFrame_single+)
  \<comment> \<open>In case of success, get corresponding results.\<close>
  apply (iApply rule: wp_cmpxchg_success[OF wp_inG, simplified, where ?l = m'], auto simp: vals_compare_safe_def)
  apply (entails_substR rule: wp_value[OF wp_inG])
  ...
  apply (iFrame2 "is_marked ?x \<^emph> cinv_own ?p \<kappa> k")
  ...
  apply (iWP rule: wp_pure[OF _ _ _ pure_exec_snd])
  apply (entails_substR rule: wp_value[OF wp_inG], iris_simp)
...
have try_mark_to_CAS:
  ...
  apply (iApply rule: wp_load[OF wp_inG, simplified, where ?l = x])
...
have split_cas: "\<lbrakk>fmlookup (of_graph g G) x = Some u; Mrk x = Some m\<rbrakk> \<Longrightarrow>
  ...
  for P Q m G u
  apply (entails_substR rule: wp_bind'[OF wp_inG, where C = Snd],iMod rule: cinv_acc[OF cinvInG.inG_axioms invInG.inG_axioms subset_UNIV])
  ...
  subgoal apply (iApply rule: wp_cmpxchg_fail[OF wp_inG, simplified, where ?l = m'], auto simp: vals_compare_safe_def)
  apply (move_sep_all True "P\<^emph>(cinv_own ?p ?c ?d)\<^emph>(?l={?E1,?E2}=\<^emph>?e)\<^emph>heap_owns ?a ?b\<^emph>Own\<^sub>m full ?gm\<^emph>Own\<^sub>g full ?G\<^emph>(x\<mapsto>\<^sub>u?x)\<^emph>(m'\<mapsto>\<^sub>u?v)")
...
for P Q y z G
  apply (entails_substR rule: wp_value[OF wp_inG], simp, remove_emp) apply (iExistsR "(y,z)", iFrame_single)
...
have cmpxchg_suc: "\<lbrakk>strict_subgraph' g (gmon_graph G'); fmlookup (of_graph g G') x = Some (False, y, z);
  ...
  apply (iFrame2 "is_marked ?x \<^emph> cinv_own ?c \<kappa> k", subst drop_marked)
  apply (iDestruct rule: auth_own_graph_valid)
  apply (iApply_step "heap_owns ?g ?m \<^emph> (x\<mapsto>\<^sub>u?v1) \<^emph> (m\<mapsto>\<^sub>u?v2)" rule: graph_close)
  ...
  apply (entails_substR rule: fupd_intro[OF invInG.inG_axioms], iApply rule: wp_pure[OF wp_inG pure_exec_snd, simplified])
