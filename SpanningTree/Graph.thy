...
type_synonym 'a graph = "('a,('a option\<times>'a option)) fmap"
type_synonym 'a graph_map = "'a\<rightharpoonup>('a option\<times>'a option)"

definition get_left :: "'a graph \<Rightarrow> 'a \<Rightarrow> 'a option" where
  "get_left g x = Option.bind (fmlookup g x) fst"
definition get_right :: "'a graph \<Rightarrow> 'a \<Rightarrow> 'a option" where
  "get_right g x = Option.bind (fmlookup g x) snd"

definition get_left' :: "'a graph_map \<Rightarrow> 'a \<Rightarrow> 'a option" where
  "get_left' g x = Option.bind (g x) fst"
definition get_right' :: "'a graph_map \<Rightarrow> 'a \<Rightarrow> 'a option" where
...
definition strict_subgraph' :: "'a graph \<Rightarrow> 'a graph_map \<Rightarrow> bool" where
  "strict_subgraph' g1 g2 \<equiv> 
    \<forall>x. strict_sub_children (get_left g1 x, get_right g1 x) (get_left' g2 x, get_right' g2 x)"
...
inductive valid_path :: "'a graph \<Rightarrow> 'a \<Rightarrow> 'a \<Rightarrow> path \<Rightarrow> bool" where
  valid_pathE: "\<lbrakk>x\<in>fmdom' g; x=y\<rbrakk> \<Longrightarrow> valid_path g x y []"
...
definition connected :: "'a graph \<Rightarrow> 'a \<Rightarrow> bool" where
  "connected g x \<equiv> \<forall>z\<in> fmdom' g. \<exists>p. valid_path g x z p"

definition front :: "'a graph \<Rightarrow> 'a set \<Rightarrow> 'a set \<Rightarrow> bool" where
  "front g t1 t2 \<equiv> t1\<subseteq>fmdom' g \<and> (\<forall>x\<in>t1. \<forall>v. ((get_left g x = Some v) \<or> (get_right g x = Some v)) \<longrightarrow> v\<in>t2)"

definition maximal :: "'a graph \<Rightarrow> bool" where "maximal g \<equiv> front g (fmdom' g) (fmdom' g)"

definition tree :: "'a graph \<Rightarrow> 'a \<Rightarrow> bool" where
  "tree g x \<equiv> \<forall>z\<in>fmdom' g. (\<exists>p. valid_path g x z p \<and> (\<forall>p'. valid_path g x z p' \<longrightarrow> (p=p')))"
