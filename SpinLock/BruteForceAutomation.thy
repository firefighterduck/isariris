...
lemmas splitting_rules = split_rule can_be_split_sepL persistent_dupl' persistent_dupl_split
...
lemmas wp_symbolic_execution_steps = upred_entails_trans[OF _ wp_pure_step_later[OF pure_exec_beta, simplified]]
  upred_entails_trans[OF _  wp_pure_step_later[OF pure_exec_fst, simplified]] 
  upred_entails_trans[OF _ wp_pure_step_later[OF pure_exec_snd, simplified]] 
...
method framing =
  ((rule single_hyp_framing, fast intro: frame_rule, iris_simp?)
...
method brute_force_hyp =
  (auto simp: iris_simp 
  intro!: pure_goal_lifting universal_elim_goal existential_elim_hyps upred_universal_wand
    intro_rules fupd_elim_hyps bupd_elim later_elim_hyps(1,2) existential_elim_goal pure_hypothesis_lifting
...
method brute_force_hyps for hyps :: iprop =
  ...
  \<bar> _ \<Rightarrow> \<open>(move_sepL hyps, brute_force_hyp)
      | step intro: last_resort_drop_goal_modality wp_symbolic_execution_steps
      | (rule framing_emp, fast intro!: log_prog_rule, is_entailment)\<close>
