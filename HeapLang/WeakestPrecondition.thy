...
definition own_state_heap :: "state \<Rightarrow> 'a::ucamera heapGCmra iprop" where
  "own_state_heap s = Own\<^sub>h (full (to_heap_op (heap s)))"

function wp2 :: "expr \<Rightarrow> (val \<Rightarrow> 'a::ucamera heapGCmra iprop) \<Rightarrow> 'a heapGCmra iprop" where
...
function wp3 :: "expr \<Rightarrow> (val \<Rightarrow> 'a::ucamera heapGCmra iprop) \<Rightarrow> 'a heapGCmra iprop" where
...
abbreviation wp :: "expr \<Rightarrow> (val \<Rightarrow> 'a::ucamera heapGCmra iprop) \<Rightarrow> 'a heapGCmra iprop" where 
  "wp \<equiv> wp3"
end