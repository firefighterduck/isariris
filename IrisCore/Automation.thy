...
method subst_pers_keep uses rule =
  (entails_substL rule: persistent_keep[OF _ rule], pers_solver)
...
method iApply_wand =
  match conclusion in \<open>hyps \<turnstile> goal\<close> for hyps goal \<Rightarrow>
    \<open>find_applicable_wand hyps hyps; subst_pers_keep rule: upred_wand_apply\<close>

method iFrame_single = match conclusion in \<open>hyps \<turnstile> goal\<close> for hyps goal :: "'a::ucamera upred_f" \<Rightarrow>
