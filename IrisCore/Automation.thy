...
method debug_goal = match conclusion in "goal" for goal :: bool \<Rightarrow> \<open>print_term goal\<close>

method iIntro = 
  (match conclusion in "upred_holds _" \<Rightarrow> 
    \<open>rule upred_wand_holdsI | subst upred_holds_entails\<close>)?,((rule upred_wandI | rule upred_implI)+)?
...
declare upred_pure_sep_conj'[iris_simp] 
  upred_pure_sep_conj''[iris_simp] 
  upred_pure_sep_conj'''[iris_simp] 
  upred_pure_sep_conj'''' [iris_simp]
...
method move_sepL for pat :: "'res::ucamera upred_f" =
  match conclusion in \<open>hyps \<turnstile> _\<close> for hyps :: "'res upred_f" \<Rightarrow>
    \<open> match (hyps) in "pat" \<Rightarrow> succeed
      ...
      \<bar> "_\<^emph>_\<^emph>_\<^emph>_\<^emph>_\<^emph>_\<^emph>_\<^emph>_\<^emph>_\<^emph>_" \<Rightarrow> \<open>rule upred_entails_trans[OF upred_sep_comm8_1]; iris_simp, move_sepL pat\<close>
    \<close>
    
method move_sepR for pat :: "'res::ucamera upred_f" =
  match conclusion in \<open>_ \<turnstile> goal\<close> for goal :: "'res upred_f" \<Rightarrow>
...
method move_sep for left :: bool and pat :: "'res::ucamera upred_f" = 
  match (left) in True \<Rightarrow> \<open>move_sepL pat\<close>
  \<bar> False \<Rightarrow> \<open>move_sepR pat\<close>
    
method move_sep_all for left :: bool and trm :: "'res::ucamera upred_f" =
  match (trm) in "rest\<^emph>head" for rest head :: "'res upred_f" \<Rightarrow> \<open>move_sep_all left rest; move_sep left head\<close>
  \<bar> "P" for P :: "'res upred_f" \<Rightarrow> \<open>move_sep left P\<close>

method move_sep_both for trm :: "'res::ucamera upred_f" =
  move_sepL trm, move_sepR trm

method move_sep_all_both for pat :: "'res::ucamera upred_f" =
  move_sep_all True pat, move_sep_all False pat
  
method find_pat_sep for pat trm :: "'res::ucamera upred_f" = 
  match (trm) in "pat" \<Rightarrow> succeed
  \<bar> "_\<^emph>pat" \<Rightarrow> succeed
  \<bar> "pat\<^emph>_" \<Rightarrow> succeed
  \<bar> "rest\<^emph>_" for rest :: "'res upred_f" \<Rightarrow> \<open>find_pat_sep pat rest\<close>   

method find_pat_conj for pat trm :: "'res::ucamera upred_f" = 
  match (trm) in "pat" \<Rightarrow> succeed
  \<bar> "_\<and>\<^sub>upat" \<Rightarrow> succeed
  \<bar> "pat\<and>\<^sub>u_" \<Rightarrow> succeed
  \<bar> "rest\<and>\<^sub>u_" for rest :: "'res upred_f" \<Rightarrow> \<open>find_pat_conj pat rest\<close>   
  
method find_in_pat_sep for pat trm :: "'res::ucamera upred_f" = 
  match (pat) in "rest\<^emph>head" for rest head :: "'res upred_f" \<Rightarrow>
    \<open> match (trm) in head \<Rightarrow> succeed | find_in_pat_sep rest trm \<close>
  \<bar> "single_pat" for single_pat :: "'res::ucamera upred_f" \<Rightarrow> 
    \<open> match (trm) in single_pat \<Rightarrow> succeed \<close>

method find_other_than_pat_sep for pat trm :: "'res::ucamera upred_f" =
  match (trm) in pat \<Rightarrow> fail
  \<bar> "\<diamondop>rest" for rest :: "'res upred_f" \<Rightarrow> \<open>find_other_than_pat_sep pat rest\<close>
  \<bar> "\<triangleright>rest" for rest :: "'res upred_f" \<Rightarrow> \<open>find_other_than_pat_sep pat rest\<close>
  \<bar> "rest\<^emph>head" for rest head :: "'res upred_f" \<Rightarrow> 
    \<open>find_other_than_pat_sep pat rest | find_other_than_pat_sep pat head\<close>
  \<bar> "_" \<Rightarrow> succeed  
  
method check_not_headL for pat :: "'res::ucamera upred_f" =
  match conclusion in "_\<^emph>head\<turnstile>_" for head :: "'res upred_f" \<Rightarrow> \<open>find_other_than_pat_sep pat head\<close>
  
method check_head for pat trm :: "'res::ucamera upred_f" =
  match (pat) in "rest_pat\<^emph>head_pat" for rest_pat head_pat :: "'res upred_f" \<Rightarrow> 
    \<open>match (trm) in "rest\<^emph>head_pat" for rest :: "'res upred_f" \<Rightarrow> \<open>check_head rest_pat rest\<close>\<close>
...
method check_headL for pat :: "'res::ucamera upred_f" =
  match conclusion in "hyps\<turnstile>_" for hyps :: "'res upred_f" \<Rightarrow> \<open>check_head pat hyps\<close>

method check_moveL for pat :: "'res::ucamera upred_f" =
  match conclusion in "hyps\<turnstile>_" for hyps :: "'res upred_f" \<Rightarrow>
    \<open>(find_pat_sep pat hyps; move_sepL pat) | find_pat_conj pat hyps\<close>
    
method check_moveR for pat :: "'res::ucamera upred_f" =
  match conclusion in \<open>_ \<turnstile> goal\<close> for goal :: "'res upred_f" \<Rightarrow>
    \<open>(find_pat_sep pat goal; move_sepR pat) | find_pat_conj pat goal\<close>
    
method check_move for left :: bool and pat :: "'res::ucamera upred_f" = 
  match (left) in True \<Rightarrow> \<open>check_moveL pat\<close>
  \<bar> False \<Rightarrow> \<open>check_moveR pat\<close>  

method check_move_all for left :: bool and trm :: "'res::ucamera upred_f" =
  match (trm) in "rest\<^emph>head" for rest head :: "'res upred_f" \<Rightarrow> 
    \<open>check_move_all left rest; check_move left head\<close>
  \<bar> "P" for P :: "'res upred_f" \<Rightarrow> \<open>check_move left P\<close>

method check_move_both for trm :: "'res::ucamera upred_f" =
  (check_moveL trm, dupl_pers), check_moveR trm
  
method check_move_dupl_all for left acc_pure :: bool and trm :: "'res::ucamera upred_f" =
  match (trm) in "rest\<^emph>\<upharpoonleft>head" for rest :: "'res upred_f" and head :: bool 
    \<Rightarrow> \<open>match (acc_pure) in True \<Rightarrow> succeed 
  ...
  \<bar> "P" for P :: "'res upred_f" \<Rightarrow> \<open>check_move left P, dupl_pers\<close>

method check_move_all_both for pat :: "'res::ucamera upred_f" =
...
method iApply uses rule = iris_simp,
  match rule[uncurry] in "hyps \<turnstile> _" for hyps
    \<Rightarrow> \<open>check_move_dupl_all True True hyps, move_sep_all True hyps, subst_pers_keepL rule: rule\<close>
  \<bar> "_ \<Longrightarrow> hyps \<turnstile> _" for hyps
    \<Rightarrow> \<open>check_move_dupl_all True True hyps, move_sep_all True hyps, subst_pers_keepL rule: rule\<close>
...
method split_pat for pat :: "'res::ucamera upred_f" = repeat_new
  \<open>((match conclusion in "can_be_split (_\<^emph>_) _ _" \<Rightarrow> succeed),
  ((rule persistent_dupl_split', log_prog_solver)
  | (((match conclusion in "can_be_split (_\<^emph>head) _ _" for head :: "'res upred_f" \<Rightarrow>
...
| ((rule persistent_dupl', log_prog_solver)
  | ((match conclusion in "can_be_split head _ _" for head :: "'res upred_f" \<Rightarrow>
...
method ord_split_pat for pat :: "'res::ucamera upred_f" =
  (match conclusion in "can_be_split (_\<^emph>_) _ _" \<Rightarrow> succeed),
  match (pat) in "rest\<^emph>_" for rest :: "'res upred_f" \<Rightarrow> 
...
method split_log_prog for pat :: "'res::ucamera upred_f" = repeat_new 
  \<open>(rule split_rule | (((match conclusion in "can_be_split head _ _" for head :: "'res upred_f" \<Rightarrow>
...
method split_move for pat :: "'res::ucamera upred_f" =
  rule upred_entails_trans[OF upred_entail_eqL[OF can_be_splitE]], split_log_prog pat,
  check_not_headL "upred_emp::'res upred_f", remove_emp, dupl_pers

method check_split_move for pat :: "'res::ucamera upred_f" =
  match conclusion in "hyps\<turnstile>_" for hyps :: "'res upred_f" \<Rightarrow>
    \<open>find_pat_sep pat hyps, split_move pat\<close>
  
method split_move_ord for pat :: "'res::ucamera upred_f" =
...
method splitO for pat :: "'res::ucamera upred_f" and goal :: "bool" = remove_emp,
  (match (goal) in "can_be_split head _ _" for head :: "'res upred_f" \<Rightarrow>
    \<open>match (head) in pat \<Rightarrow> \<open>rule can_be_split_baseR\<close>\<close>) (* Pattern found *)
| match (goal) in "can_be_split (\<triangleright>head) _ _" for head :: "'res upred_f" \<Rightarrow>
    \<open>match (pat) in "\<triangleright>rest" for rest :: "'res upred_f" \<Rightarrow> \<open>rule can_be_split_later, 
      splitO rest "can_be_split head ?l ?r"\<close> \<bar> _ \<Rightarrow> \<open>rule can_be_split_baseL\<close>\<close>
  \<bar> "can_be_split (\<diamondop>head) _ _" for head :: "'res upred_f" \<Rightarrow>
    \<open>match (pat) in "\<diamondop>rest" for rest :: "'res upred_f" \<Rightarrow> \<open>rule can_be_split_except_zero,
      splitO rest "can_be_split head ?l ?r"\<close> \<bar> _ \<Rightarrow> \<open>rule can_be_split_baseL\<close>\<close>
  \<bar> "can_be_split (tail\<^emph>head) _ _" for head tail :: "'res upred_f" \<Rightarrow> 
    \<open>rule can_be_split_mono, splitO pat "can_be_split tail ?l ?r", splitO pat "can_be_split head ?l ?r"\<close>
  \<bar> "can_be_split (l\<or>\<^sub>ur) _ _" for l r :: "'res upred_f" \<Rightarrow> 
...
method split_moveO for pat :: "'res::ucamera upred_f" =
  match conclusion in "hyps\<turnstile>_" for hyps :: "'res upred_f" \<Rightarrow> 
  \<open>rule upred_entails_trans[OF upred_entail_eqL[OF can_be_splitE]],
   splitO pat "can_be_split hyps ?l ?r", remove_emp\<close>

method check_split_moveO for pat :: "'res::ucamera upred_f" =
  match conclusion in "hyps\<turnstile>_" for hyps :: "'res upred_f" \<Rightarrow>
    \<open>find_pat_sep pat hyps, split_moveO pat\<close>
  
method split_move_allO for pat :: "'res::ucamera upred_f" =
  match (pat) in "tail_pat\<^emph>head_pat" for tail_pat head_pat :: "'res upred_f" \<Rightarrow>
...
method iApply_step' for pat :: "'res::ucamera upred_f" uses rule = iris_simp,
...
method iApply_step2' for step pat :: "'res::ucamera upred_f" = iris_simp,
...
method iApply_wand_as_rule for lhs_pat pat :: "'res::ucamera upred_f" = iris_simp, 
  check_moveL "lhs_pat-\<^emph>?a_schematic_variable_name_that_noone_would_use_in_setp_pat";
  match conclusion in "_\<^emph>(step_trm-\<^emph>P)\<turnstile>_" for step_trm P :: "'res upred_f" \<Rightarrow>
    \<open>iApply_step2 step_trm pat,
    prefer_last,
      move_sep_all True step_trm, move_sepL "step_trm-\<^emph>P", subst_pers_keepL rule: upred_wand_apply[of step_trm],
    defer_tac\<close>, iris_simp
  
method iExistsL = iris_simp,
  (match conclusion in "(_::'res::ucamera upred_f) \<turnstile> (_::'res upred_f)" \<Rightarrow>
  \<open>check_split_moveO "(upred_exists (?P::_ \<Rightarrow> 'res upred_f))"\<close>)?, 
  ((unfold pull_exists_antecedentR' | rule pull_exists_antecedentR)+)?;
  rule upred_existsE', iris_simp

method iExistsR for inst :: "'b" = iris_simp,
  match conclusion in "(_::'res::ucamera upred_f) \<turnstile> (_::'res upred_f)" \<Rightarrow>
  \<open>check_moveR "(upred_exists (?P::'b \<Rightarrow> 'res upred_f))"\<close>; (unfold pull_exists_eq pull_exists_eq')?; 
  rule upred_existsI[of _ _ inst], iris_simp

method iForallL for inst :: "'b" = iris_simp,
  match conclusion in "(_::'res::ucamera upred_f) \<turnstile> (_::'res upred_f)" \<Rightarrow>
  \<open>check_moveL "(upred_forall (?P::'b \<Rightarrow> 'res upred_f))"\<close>;
  rule upred_entails_trans[OF upred_forall_inst[of _ inst]] upred_entails_substE[OF upred_forall_inst[of _ inst]], 
  iris_simp
  
method iForallR = iris_simp, match conclusion in "(_::'res::ucamera upred_f) \<turnstile> (_::'res upred_f)" \<Rightarrow>
  \<open>check_moveR "(upred_forall (?P::_ \<Rightarrow> 'res upred_f))"\<close>; rule upred_forallI, iris_simp
  
method iPureL =
  iris_simp, match conclusion in "(_::'res::ucamera upred_f) \<turnstile> (_::'res upred_f)" \<Rightarrow>
  \<open>check_moveL "(upred_pure ?b)::'res upred_f"\<close>, rule upred_pure_impl, iris_simp

method iPureR = 
  iris_simp, match conclusion in "(_::'res::ucamera upred_f) \<turnstile> (_::'res upred_f)" \<Rightarrow>
  \<open>check_moveR "(upred_pure ?b)::'res upred_f"\<close>, rule upred_pureI' upred_pureI, assumption?, iris_simp
  
method find_applicable_wand for trm :: "'res::ucamera upred_f" =
  match (trm) in "P-\<^emph>Q" for P Q :: "'res upred_f" \<Rightarrow>
    \<open>check_moveL P, dupl_pers, move_sepL "P-\<^emph>Q"\<close>
  \<bar> "rest\<^emph>(P-\<^emph>Q)" for P Q rest :: "'res upred_f" \<Rightarrow>
    \<open>(check_moveL P, dupl_pers, move_sepL "P-\<^emph>Q") | find_applicable_wand rest\<close>
  \<bar> "rest\<^emph>_" for rest :: "'res upred_f" \<Rightarrow> \<open>find_applicable_wand rest\<close>

method iApply_wand = iris_simp,
  match conclusion in \<open>hyps \<turnstile> _\<close> for hyps :: "'res::ucamera upred_f" \<Rightarrow>
...
method iFrame_single_safe for trm :: bool = iris_simp,
  match (trm) in \<open>_ \<turnstile> goal\<close> for goal :: "'res::ucamera upred_f" \<Rightarrow>
    \<open> match (goal) in "_\<^emph>P" for P :: "'res upred_f" \<Rightarrow> 
...
method iExistsR2 = iris_simp,
  (match conclusion in "(_::'res::ucamera upred_f) \<turnstile> (_::'res upred_f)" \<Rightarrow>
  \<open>check_moveR "(upred_exists (?P::_ \<Rightarrow> 'res upred_f))"\<close>; (unfold pull_exists_eq pull_exists_eq')?; 
  rule upred_exists_lift, rule exI)+, iris_simp

method later_elim = iris_simp,
  match conclusion in "(?l::'res::ucamera upred_f) \<turnstile> (?r::'res upred_f)" \<Rightarrow>
  \<open>check_moveL "\<triangleright> (?P::'res upred_f)"\<close>, 
...
method iMod_raw_wand for lhs_pat pat :: "'res::ucamera upred_f" methods later fupd =
  iris_simp, iApply_wand_as_rule lhs_pat pat, (prefer_last, (later | fupd)+, defer_tac)?, iris_simp

method iMod_raw_step for pat :: "'res::ucamera upred_f" methods later fupd uses rule = 
  iris_simp, iApply_step pat rule: rule; all \<open>((later | fupd)+)?\<close>; iris_simp
  
method iDestruct_raw = iris_simp;
  ((check_moveL "\<V> (?c::_::dcamera)", entails_substL rule: upred_entail_eqL[OF discrete_valid], rule upred_pure_impl)
| iPureL
| iExistsL); iris_simp

method iIntros = 
  (match conclusion in "upred_holds _" \<Rightarrow> \<open>rule upred_wand_holdsI | subst upred_holds_entails\<close>)?,
  ((rule upred_wandI upred_implI | iForallR | iPureR | iExistsL | iPureL | (rule persistent_persisI,pers_solver)), 
    iris_simp)+
...
method iDrop for pat :: "'res::ucamera upred_f" =
  iris_simp, check_moveL pat; rule upred_entails_trans[OF upred_weakeningL], iris_simp

method iSplit for lpat rpat :: "'res::ucamera upred_f" = 
  match conclusion in "hyps\<turnstile>goal" for hyps goal :: "'res upred_f" \<Rightarrow> 
  \<open>rule split_frame, split_pat lpat, splitO rpat "can_be_split goal ?l ?r",
    iris_simp\<close>

method iSplitR for lpat rpat :: "'res::ucamera upred_f" = 
  match conclusion in "hyps\<turnstile>goal" for hyps goal :: "'res upred_f" \<Rightarrow> 
  \<open>rule split_frame[OF can_be_split_rev can_be_split_rev], 
    split_pat lpat, splitO rpat "can_be_split goal ?l ?r", iris_simp\<close>
  
method iNext = entails_substR rule: upred_laterI

method iAssert_pers for pat :: "'res::ucamera upred_f" = 
  rule persistent_add[where ?Q=pat], log_prog_solver

method frame_single =
  rule upred_entails_refl | rule upred_weakeningR | rule upred_weakeningL
| (rule framing, (log_prog_solver | frame_solver))
| (rule framing_emp, (log_prog_solver | frame_solver))
