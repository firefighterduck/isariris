...
abbreviation upred :: "'a::ucamera upred_f \<Rightarrow> 'a upred" where "upred P \<equiv> Upred P \<epsilon>"
abbreviation pred_f :: "('a::ucamera,'b) upred_t \<Rightarrow> 'a \<Rightarrow> nat \<Rightarrow> bool" where
  "pred_f P \<equiv> Rep_upred_f (pred P)"

instantiation upred_t :: (ucamera,type) ofe begin
  definition n_equiv_upred_t :: "nat \<Rightarrow> ('a, 'b) upred_t \<Rightarrow> ('a, 'b) upred_t \<Rightarrow> bool" where [simp]:
    "n_equiv_upred_t n P Q = n_equiv n (pred P) (pred Q)"
  definition ofe_eq_upred_t :: "('a, 'b) upred_t \<Rightarrow> ('a, 'b) upred_t \<Rightarrow> bool" where [simp]:
    "ofe_eq_upred_t P Q = ofe_eq (pred P) (pred Q)"
instance by standard (auto simp: ofe_refl ofe_sym ofe_limit ofe_eq_eq ofe_mono intro: ofe_trans)
end

lift_definition upred_chain :: "('a::ucamera,'b) upred_t chain \<Rightarrow> 'a upred_f chain" is
  "\<lambda>c. pred \<circ> (Rep_chain c)"
  by (metis (mono_tags, lifting) Rep_chain mem_Collect_eq n_equiv_upred_t_def o_def)

instantiation upred_t :: (ucamera,ucamera) cofe begin
  definition lim_upred_t :: "('a, 'b) upred_t chain \<Rightarrow> ('a, 'b) upred_t" where [simp]:
    "lim_upred_t c = Upred (lim (upred_chain c)) \<epsilon>"
instance apply standard apply (simp add: core_compl upred_chain_def pred_def split: upred_t.splits)
  by (metis comp_def core_compl upred_t.sel(1) upred_chain.abs_eq upred_chain.rep_eq)
end
...
private lift_definition upred_f_holds :: "'a upred_f \<Rightarrow> bool" is
  "\<lambda>u. \<forall>a n. n_valid a n \<longrightarrow>  u a n" .

definition upred_holds :: "'a upred \<Rightarrow> bool" where
  "upred_holds P \<equiv> \<forall>a n. n_valid a n \<longrightarrow> pred_f P a n"

private lemma "upred_holds P \<longleftrightarrow> upred_f_holds (pred P)"
  by (auto simp: upred_holds_def upred_f_holds_def)
...
subsubsection \<open>Uniform predicate semantics\<close>
text \<open> The semantic of uniform predicates is defined as a shallow embedding. \<close>

lift_definition upred_f_pure :: "bool \<Rightarrow> 'a upred_f" is "\<lambda>b::bool. \<lambda>_::'a. \<lambda>_::nat. b" .
abbreviation upred_pure :: "bool \<Rightarrow> 'a upred" ("\<upharpoonleft>_") where "upred_pure b \<equiv> upred (upred_f_pure b)"

lift_definition upred_f_eq :: "'c::ofe \<Rightarrow> 'c \<Rightarrow> 'a upred_f" is 
  "\<lambda>x (y::'c). \<lambda>_::'a. \<lambda>n. ofe_class.n_equiv n x y" by (rule ofe_mono)
abbreviation upred_eq :: "'c::ofe \<Rightarrow> 'c \<Rightarrow> 'a upred" (infix "=\<^sub>u" 60) where
  "upred_eq x y \<equiv> upred (upred_f_eq x y)"
  
lift_definition upred_f_conj :: "'a upred_f \<Rightarrow> 'a upred_f \<Rightarrow> 'a upred_f" is
  "\<lambda>P Q (a::'a) (n::nat). P a n \<and> Q a n" by (rule conj_forward)
abbreviation upred_conj :: "'a upred \<Rightarrow> 'a upred \<Rightarrow> 'a upred" (infixl "\<and>\<^sub>u" 60) where 
  "upred_conj P Q \<equiv> upred (upred_f_conj (pred P) (pred Q))"

lift_definition upred_f_disj :: "'a upred_f \<Rightarrow> 'a upred_f \<Rightarrow> 'a upred_f" is
  "\<lambda>P Q (a::'a) (n::nat). P a n \<or> Q a n" by (rule disj_forward)
abbreviation upred_disj :: "'a upred \<Rightarrow> 'a upred \<Rightarrow> 'a upred" (infixl "\<or>\<^sub>u" 60) where 
  "upred_disj P Q \<equiv> upred (upred_f_disj (pred P) (pred Q))"
  
lift_definition upred_f_impl :: "'a upred_f \<Rightarrow> 'a upred_f \<Rightarrow> 'a upred_f" is
...
proof -
  ...
  show "y c m".
qed
abbreviation upred_impl :: "'a upred \<Rightarrow> 'a upred \<Rightarrow> 'a upred" (infixr "\<longrightarrow>\<^sub>u" 60) where
  "upred_impl P Q \<equiv> upred (upred_f_impl (pred P) (pred Q))"

lift_definition upred_f_forall :: "('c \<Rightarrow> 'a upred_f) \<Rightarrow> 'a upred_f" is 
  "\<lambda>P (a::'a) (n::nat). \<forall>x::'c. P x a n" by (rule meta_allE)
abbreviation upred_forall :: "('c \<Rightarrow> 'a upred) \<Rightarrow> 'a upred" (binder "\<forall>\<^sub>u" 60) where
  "upred_forall P \<equiv> upred (upred_f_forall (pred \<circ> P))"
  
lift_definition upred_f_exists :: "('c \<Rightarrow> 'a upred_f) \<Rightarrow> 'a upred_f" is 
  "\<lambda>P (a::'a) (n::nat). \<exists>x::'c. P x a n" by (rule ex_forward)
abbreviation upred_exists :: "('c \<Rightarrow> 'a upred) \<Rightarrow> 'a upred" (binder "\<exists>\<^sub>u" 60) where
  "upred_exists P \<equiv> upred (upred_f_exists (pred \<circ> P))"
  
lift_definition upred_f_sep :: "'a upred_f \<Rightarrow> 'a upred_f \<Rightarrow> 'a upred_f" is
...
qed
abbreviation upred_sep :: "'a upred \<Rightarrow> 'a upred \<Rightarrow> 'a upred" (infixl "\<^emph>" 80) where
  "upred_sep P Q \<equiv> upred (upred_f_sep (pred P) (pred Q))"

lift_definition upred_f_wand :: "'a upred_f \<Rightarrow> 'a upred_f \<Rightarrow> 'a upred_f" is 
...
proof -
  ...
  show "Q (op b c) m" .
qed
abbreviation upred_wand :: "'a upred \<Rightarrow> 'a upred \<Rightarrow> 'a upred" (infixr "-\<^emph>" 60) where
  "upred_wand P Q \<equiv> upred (upred_f_wand (pred P) (pred Q))"

lift_definition upred_f_own :: "'a \<Rightarrow> 'a upred_f" is "\<lambda>(a::'a) b n. n_incl n a b" 
  unfolding n_incl_def using camera_assoc op_equiv_subst by metis
abbreviation upred_own :: "'a \<Rightarrow> 'a upred" ("Own(_)") where "upred_own x \<equiv> upred (upred_f_own x)"
  
lift_definition upred_f_valid :: "'b::camera \<Rightarrow> 'a upred_f" is "\<lambda>(a::'b) (_::'a). n_valid a" 
  using Rep_sprop n_incl_def by blast
abbreviation upred_valid :: "'b::camera \<Rightarrow> 'a upred" ("\<V>(_)") where 
  "upred_valid x \<equiv> upred (upred_f_valid x)"
  
lift_definition upred_f_persis :: "'a upred_f \<Rightarrow> 'a upred_f" is "\<lambda>P (a::'a) n. (P (core a) n)::bool"
...
qed
abbreviation upred_persis :: "'a upred \<Rightarrow> 'a upred" ("\<box>_") where 
  "upred_persis P \<equiv> upred (upred_f_persis (pred P))"

lift_definition upred_f_plain :: "'a upred_f \<Rightarrow> 'a upred_f" is "\<lambda>P (_::'a) (n::nat). (P (\<epsilon>::'a) n)::bool"
  by (metis \<epsilon>_left_id ofe_eq_limit n_incl_def)
abbreviation upred_plain :: "'a upred \<Rightarrow> 'a upred" ("\<^item>_") where 
  "upred_plain P \<equiv> upred (upred_f_plain (pred P))"
  
lift_definition upred_f_later :: "'a upred_f \<Rightarrow> 'a upred_f" is "\<lambda>P (a::'a) n. n=0 \<or> P a (n-1)" 
  by (smt (verit, ccfv_SIG) n_incl_def diff_0_eq_0 diff_diff_cancel diff_is_0_eq diff_right_commute diff_self_eq_0 ofe_mono)
abbreviation upred_later :: "'a upred \<Rightarrow> 'a upred" ("\<triangleright>_") where
  "upred_later P \<equiv> upred (upred_f_later (pred P))"  
  
lift_definition upred_f_bupd :: "'a upred_f \<Rightarrow> 'a upred_f" is
...
abbreviation upred_bupd :: "'a upred \<Rightarrow> 'a upred" ("\<Rrightarrow>\<^sub>b_") where
  "upred_bupd P \<equiv> upred (upred_f_bupd (pred P))"
  
lift_definition upred_f_entails :: "'a upred_f \<Rightarrow> 'a upred_f \<Rightarrow> bool" is
  "\<lambda>P Q. \<forall>(a::'a) n. n_valid a n \<longrightarrow> P a n \<longrightarrow> Q a n" .
abbreviation upred_entails :: "'a upred \<Rightarrow> 'a upred \<Rightarrow> bool" (infix "\<turnstile>" 50) where
  "upred_entails P Q \<equiv> upred_f_entails (pred P) (pred Q)"
  
definition upred_entail_eq :: "'a upred \<Rightarrow> 'a upred \<Rightarrow> bool" (infix "\<stileturn>\<turnstile>" 40) where
  "P \<stileturn>\<turnstile> Q \<equiv> (upred_entails P Q) \<and> (upred_entails Q P)"
end

(* Basic view shift operator *)
definition upred_bvs :: "('a::ucamera) upred \<Rightarrow> 'a upred \<Rightarrow> 'a upred" (infix "\<Rrightarrow>\<^sub>v" 60) where
  "upred_bvs P Q \<equiv> \<box>(P -\<^emph> (\<Rrightarrow>\<^sub>b Q))"

subsubsection \<open> Basic properties of upred predicates: \<close>

lemma upred_entail_eq_simp: "P\<stileturn>\<turnstile>Q \<equiv> \<forall>a n. n_valid a n \<longrightarrow> pred_f P a n = pred_f Q a n"
  by (auto simp: upred_entail_eq_def upred_f_entails.rep_eq) (smt (verit, best))
lemma upred_entail_eqE: "P\<stileturn>\<turnstile>Q \<Longrightarrow> (\<And>a n. n_valid a n \<Longrightarrow> pred_f P a n = pred_f Q a n)"
  using upred_entail_eq_simp by auto
lemma upred_entail_eqI: "(\<And>a n. n_valid a n \<Longrightarrow> pred_f P a n = pred_f Q a n) \<Longrightarrow> P\<stileturn>\<turnstile>Q"
  using upred_entail_eq_simp by auto
...
lemma upred_entails_trans: "\<lbrakk>P\<turnstile>Q; Q\<turnstile>R\<rbrakk> \<Longrightarrow> P\<turnstile>R"
  by (auto simp: upred_f_entails.rep_eq)

lemma upred_entails_refl [simp]: "P\<turnstile>P" by (auto simp: upred_f_entails_def)

lemma own_valid: "Own(a) \<turnstile> \<V>(a)"
  by (metis upred_t.sel(1) upred_f_entails.rep_eq upred_f_own.rep_eq upred_f_valid.rep_eq n_incl_def 
    camera_valid_op n_valid_ne)
  
lemma upred_holds_entails: "upred_holds P \<longleftrightarrow> ((\<upharpoonleft>True) \<turnstile> P)"
  by (auto simp: upred_holds_def upred_f_entails.rep_eq upred_f_pure.rep_eq)

lemma upred_entailsE: "P \<turnstile> Q \<Longrightarrow> (\<And>a n. \<lbrakk>n_valid a n; pred_f P a n\<rbrakk> \<Longrightarrow> pred_f Q a n)"
  by (auto simp: upred_f_entails.rep_eq)

lemma upred_wandI: "(P \<^emph> Q) \<turnstile> R \<Longrightarrow> P \<turnstile> (Q-\<^emph>R)"
  by (smt (verit, del_insts) upred_t.sel(1) upred_f_entails.rep_eq upred_f_sep.rep_eq 
    upred_f_wand.rep_eq ofe_refl upred_def_rep upred_weaken_simple)
lemma upred_wandE: "P \<turnstile> (Q-\<^emph>R) \<Longrightarrow> (P \<^emph> Q) \<turnstile> R"
  by (smt (verit, best) upred_def_rep upred_f_entails.rep_eq upred_f_sep.rep_eq upred_f_wand.rep_eq
    upred_t.sel(1) upred_weaken camera_valid_op dual_order.refl n_valid_ne ofe_sym total_n_inclI)

lemma upred_true_sep: "(P \<^emph> \<upharpoonleft>True) = Upred (pred P) \<epsilon>"
proof -
  have "upred_f_sep p (upred_f_pure True) = p" for p::"'a upred_f"
  apply transfer using n_incl_def by fastforce
  then show ?thesis by simp
qed

lemma upred_true_sep': "ghost P = \<epsilon> \<Longrightarrow> (P \<^emph> \<upharpoonleft>True) = P" 
  by (metis upred_t.collapse upred_true_sep)

lemma upred_sep_comm: "P \<^emph> Q = Q \<^emph> P"
proof -
  have "upred_f_sep p q = upred_f_sep q p" for p q :: "'a upred_f"
...
lemma upred_sep_mono: "\<lbrakk>P1\<turnstile>Q;P2\<turnstile>R\<rbrakk> \<Longrightarrow> P1\<^emph>P2\<turnstile>Q\<^emph>R"
  by (metis (no_types, opaque_lifting) upred_entails_refl upred_entails_trans upred_sep_comm 
    upred_wandE upred_wandI)
...
lemma upred_sep_pure: "\<lbrakk>P\<turnstile>Q;P\<turnstile>\<upharpoonleft>b\<rbrakk> \<Longrightarrow> P\<turnstile>Q\<^emph>\<upharpoonleft>b"
  by (smt (z3) upred_f_entails.rep_eq upred_f_pure.rep_eq upred_t.sel(1) upred_true_sep)

lemma upred_wand_holdsI: "Q \<turnstile> R \<Longrightarrow> upred_holds (Q-\<^emph>R)"
  by (metis upred_holds_entails upred_sep_comm upred_t.sel(1) upred_true_sep upred_wandI)
lemma upred_wand_holdsE: "upred_holds (Q-\<^emph>R) \<Longrightarrow> Q \<turnstile> R"
  by (metis upred_holds_entails upred_sep_comm upred_t.sel(1) upred_true_sep upred_wandE)

lemma upred_own_core_persis: "Own(a) \<turnstile> \<box>Own(core a)"
  by (auto simp: upred_f_entails.rep_eq upred_f_own.rep_eq upred_f_persis.rep_eq camera_core_mono_n)

lemma upred_entails_wand_holdsL: "\<lbrakk>P \<turnstile> Q; upred_holds (Q-\<^emph>R)\<rbrakk> \<Longrightarrow> upred_holds (P-\<^emph>R)"
  using upred_entails_trans upred_wand_holdsE upred_wand_holdsI by blast
...
lemma upred_entails_wand_holdsR2: "\<lbrakk>Q \<turnstile> R; upred_holds (P1-\<^emph>P2-\<^emph>Q)\<rbrakk> \<Longrightarrow> upred_holds (P1-\<^emph>P2-\<^emph>R)"
  by (meson upred_entails_trans upred_wandE upred_wandI upred_wand_holdsE upred_wand_holdsI)

lemma pure_entailsI: "(p \<Longrightarrow> q) \<Longrightarrow> \<upharpoonleft>p\<turnstile>\<upharpoonleft>q"
  by (auto simp: upred_f_pure_def upred_f_entails.rep_eq Abs_upred_f_inverse)

lemma discrete_valid: "\<V>(a::'a::dcamera) \<stileturn>\<turnstile> \<upharpoonleft>(valid a)"
  apply (rule upred_entail_eqI; auto simp: upred_f_valid.rep_eq upred_f_pure.rep_eq)
...
lemma own_op: "Own(a\<cdot>b) \<stileturn>\<turnstile> Own(a) \<^emph> Own(b)"
  ...
  apply (simp add: upred_f_own.rep_eq upred_f_sep.rep_eq)
  apply (rule iffI)
  apply (metis camera_assoc n_incl_def n_incl_refl)
  apply (erule exE)+
  by (smt (verit, del_insts) camera_comm le_cases3 n_incl_extend upred_def_rep upred_equiv_upred_f 
    upred_f_own.rep_eq)
...
lemma entails_pure_extend: "\<lbrakk>P\<turnstile>\<upharpoonleft>b;b \<Longrightarrow> P\<turnstile>Q\<rbrakk> \<Longrightarrow> P\<turnstile>Q"
  by (metis upred_f_entails.rep_eq upred_f_pure.rep_eq upred_t.sel(1))
...
lemma upred_own_nothing_true: "Own \<epsilon> \<stileturn>\<turnstile> \<upharpoonleft>True"
  by (rule upred_entail_eqI) (auto simp: upred_f_pure.rep_eq upred_f_own.rep_eq)

subsubsection \<open> Persistent predicates \<close>
definition persistent :: "('a::ucamera) upred \<Rightarrow> bool" where "persistent P \<equiv> P \<turnstile> \<box>P"
...
lemma persistent_holds_sep: 
  ...
  by (simp add: persistent_def upred_holds_def upred_f_entails.rep_eq upred_f_persis.rep_eq 
      upred_f_sep.rep_eq) (smt (z3) camera_core_id ofe_refl upred_f_pure.rep_eq upred_f_sep.rep_eq 
      upred_sep_comm upred_t.sel(1) upred_true_sep)
  
lemma persistent_persis: "persistent (\<box>P)"
  by (auto simp: persistent_def upred_f_persis.rep_eq upred_f_entails.rep_eq camera_core_idem)

lemma persistent_pure: "persistent (\<upharpoonleft>P)"
  by (auto simp: persistent_def upred_f_persis.rep_eq upred_f_entails.rep_eq upred_f_pure.rep_eq)  

lemma persistent_valid: "persistent (\<V>(a))"
  by (auto simp: persistent_def upred_f_persis.rep_eq upred_f_entails.rep_eq upred_f_valid.rep_eq)

lemma persistent_core_own: "persistent (Own(a::'a::{core_id,ucamera}))"
  by (auto simp: persistent_def upred_f_persis.rep_eq upred_f_entails.rep_eq upred_f_own.rep_eq core_id)

lemma persistent_core_own2: "pcore_id_pred (a::'a::ucamera) \<Longrightarrow> persistent (Own a)"
  apply (simp add: persistent_def upred_f_persis.rep_eq upred_f_entails.rep_eq upred_f_own.rep_eq core_id_pred)
...
lemma persistent_conj: "\<lbrakk>persistent P; persistent Q\<rbrakk> \<Longrightarrow> persistent (P \<and>\<^sub>u Q)"
  by (auto simp: persistent_def upred_f_conj.rep_eq upred_f_entails.rep_eq upred_f_persis.rep_eq)
  
lemma persistent_exists: "\<forall>x. persistent (P x) \<Longrightarrow> persistent (\<exists>\<^sub>u x. P x)"
  by (auto simp: upred_f_exists.rep_eq persistent_def upred_f_persis.rep_eq upred_f_entails.rep_eq)

subsubsection \<open> Timeless predicates \<close>
definition except_zero :: "('a::ucamera) upred \<Rightarrow> 'a upred" ("\<diamondop>_") where 
  "except_zero P \<equiv> P \<or>\<^sub>u \<triangleright>\<upharpoonleft>False"
definition timeless :: "('a::ucamera) upred \<Rightarrow> bool" where "timeless P \<equiv> (\<triangleright>P) \<turnstile> \<diamondop>P"
end